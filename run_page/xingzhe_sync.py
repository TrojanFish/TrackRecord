...
class C:
    ...
    B = "\033[94m"; E = "\033[91m";  D = "\033[2m"

def log(msg):  print(f"{C.B}->  {msg}{C.R}")
def ok(msg):   print(f"{C.G}OK  {msg}{C.R}")
def warn(msg): print(f"{C.Y}!!  {msg}{C.R}")
def err(msg):  print(f"{C.E}XX  {msg}{C.R}")
...
class XingzheClient:
    ...
    def login_by_account(self, account, password):
        ...
        try:
            ...
            self.user_id = str(res.get("user_id") or res.get("userid") or res.get("id") or "")
            ...
            if not self.user_id:
                try:
                    r_user = self.session.get(f"{BASE_URL}/api/v4/account/get_user_info/", timeout=10)
                    if r_user.status_code == 200:
                        user_info = r_user.json()
                        # Some versions use a nested data object
                        data = user_info.get("data") or user_info
                        self.user_id = str(data.get("user_id") or data.get("userid") or data.get("id") or "")
                except:
                    pass
    ...
    def auth_by_session(self, sessionid, user_id):
        ...
        self.session.cookies.set("_XingzheWeb_Token", "true", domain="www.imxingzhe.com")
    ...
    def get_workouts_by_month(self, year, month):
        ...
        offset  = 0
        while True:
            params = {"offset": offset, "limit": LIST_LIMIT,
...
def build_gpx_from_stream(workout_id, title, stream_data):
    ...
    for i in range(n):
        ...
        if hr:
            ...
            tpe   = etree.SubElement(ext, "{http://www.garmin.com/xmlschemas/TrackPointExtension/v1}TrackPointExtension")
            hr_el = etree.SubElement(tpe, "{http://www.garmin.com/xmlschemas/TrackPointExtension/v1}hr")
...
def workout_to_activity(w):
    ...
    try:
        ...
        name         = (w.get("title")
                        or f"Xingzhe {act_type} {start_dt.strftime('%Y-%m-%d')}")
...
def save_to_db(activities):
    if not HAS_GENERATOR:
        ...
        return
    try:
        generator = Generator("xingzhe")
        generator.sync(activities)
...
def main():
    ...
    parser.add_argument("--session",   metavar="SESSIONID",
                        help="浏览器 Cookie 中的 sessionid 值 (可选)")
    parser.add_argument("--user-id",   metavar="USER_ID",
                        help="行者用户 ID (可选)")
    parser.add_argument("--with-gpx",  action="store_true",
                        help="同时将轨迹导出为 GPX 文件到 GPX_OUT/")
    parser.add_argument("--gpx-only",  action="store_true",
                        help="只导出 GPX，不写入数据库")
    parser.add_argument("--only-ride", action="store_true",
                        help="只处理骑行类型")
